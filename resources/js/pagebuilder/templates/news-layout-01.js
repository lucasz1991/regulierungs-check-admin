...
export const newsLayoutHtml = [
    ...
    // Auf Maus-/Desktop-Geraeten kopiert der zentrale Teilen-Handler die URL
    // direkt; auf Touch-Geraeten nutzt er weiterhin die Web-Share-API. Das
    // Skript dafuer liegt in der Base-View news-show - die Vorlage selbst darf
    // keine Skripte enthalten, weil der HTML-Parser des Editors sie verwerfen
    // wuerde.
    '      <a href="#" data-share="native" data-desktop-share="clipboard" aria-label="Artikel teilen" title="Auf Desktop Link kopieren, auf Mobilgeräten teilen" style="display:flex;min-width:0;min-height:clamp(52px,12vw,58px);flex-wrap:wrap;align-items:center;justify-content:center;gap:clamp(4px,1.2vw,9px);padding:clamp(8px,2vw,10px) clamp(6px,2vw,16px);border-left:1px solid #e5eaee;color:#344554;font-size:clamp(11px,2.6vw,13px);font-weight:600;line-height:1.3;text-align:center;text-decoration:none;overflow-wrap:break-word"><i class="fa-light fal fa-share-alt" aria-hidden="true" style="display:inline-grid;width:clamp(18px,4.6vw,25px);height:clamp(18px,4.6vw,25px);flex:0 0 auto;place-items:center;font-size:clamp(15px,3.8vw,20px)"></i><span>Artikel teilen</span></a>',
    '    </section>',
    '',
