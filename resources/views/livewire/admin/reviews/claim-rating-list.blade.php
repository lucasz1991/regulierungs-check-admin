<div @if($hasActiveRating) wire:poll.500ms @endif x-data="{ selectedRatings: @entangle('selectedRatings'), search: @entangle('search'), hasRatings: @entangle('hasRatings') }">
    <div class="mb-6 border-l-4 border-blue-500 bg-blue-50 p-4 text-blue-700">
    ...
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500">Insgesamt {{ $ratings->total() }} Eintr&auml;ge</p>
        </div>
        <div>
    ...
    <div x-data="{ focused: false }" @click.away="focused = false" x-cloak class="relative my-6">
        <div
            class="flex items-center rounded-full border border-gray-300 ring ring-offset-4 transition-all duration-300"
            :class="{
        ...
        >
            ...
            />
            <div
                @click="focused = true; $refs.searchInput.focus()"
                class="flex h-[40px] w-[40px] cursor-pointer items-center justify-center text-gray-400 hover:text-gray-500"
            >
        ...
        <x-slot:header>
            ...
            <div data-role="datatable-heading" role="columnheader" class="flex min-w-0 items-center">
                <button wire:click="sortByField('name')" class="flex items-center text-left">
                    Kunde
                    @if ($sortBy === 'user.name')
                        <span class="ml-2 text-xl" style="display: inline-block;">
                            <svg class="ml-2 h-4 w-4 transform transition-transform" style="transform: rotate({{ $sortDirection === 'asc' ? '0deg' : '180deg' }}); transition: transform 0.3s ease;" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            >
                ...
                >
                    ...
                    <span class="flex items-center space-x-3">
                        ...
                        <span>
                            @if($rating->is_public)
                                <span class="mr-1 inline-block h-2 w-2 rounded-full bg-green-200 text-green-400" title="&Ouml;ffentliche Bewertung"></span>
                            @else
                                <span class="mr-1 inline-block h-2 w-2 rounded-full bg-gray-200 text-gray-400" title="Private Bewertung"></span>
                            @endif
                            {{ $rating->id ?? '-' }}
                ...
                <div data-role="datatable-cell" data-label="Status" role="cell" class="relative min-w-[220px] text-gray-600">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <div x-data="{ open: false }" class="relative shrink-0">
                            <button @click="open = !open" class="rounded-full p-2 text-gray-500 transition duration-200 scale-100 hover:scale-120 hover:bg-gray-100 hover:text-gray-800 focus:bg-gray-100">
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak class="absolute right-0 z-50 mt-2 w-40 rounded border bg-white text-left text-sm shadow">
                                <a href="{{ route('admin.reviews.show', ['ratingId' => $rating->id]) }}" class="block w-full px-4 py-2 text-left hover:bg-blue-100">&Ouml;ffnen</a>
                                <button wire:click.stop="reanalyse({{ $rating->id }})" class="block w-full px-4 py-2 text-left hover:bg-blue-100">Neu analysieren</button>
                                <button wire:click="toggleRatingSelection({{ $rating->id }})" class="block w-full px-4 py-2 text-left hover:bg-blue-100">ausw&auml;hlen</button>
                                @if($rating->user?->isAdmin())
                                    @if($rating->is_public)
                                        <button wire:click.stop="makePrivate({{ $rating->id }})" class="block w-full px-4 py-2 text-left text-yellow-700 hover:bg-yellow-100">Privat machen</button>
                                    @else
                                        <button wire:click.stop="makePublic({{ $rating->id }})" class="block w-full px-4 py-2 text-left text-green-700 hover:bg-green-100">&Ouml;ffentlich machen</button>
                                    @endif
                                @endif
                                <button wire:click.stop="deleteRating({{ $rating->id }})" class="block w-full px-4 py-2 text-left text-red-700 hover:bg-red-100">L&ouml;schen</button>
                            </div>
                        </div>
                    </div>
                </div>
